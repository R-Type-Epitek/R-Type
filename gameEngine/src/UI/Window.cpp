...
namespace GameEngine::UI
{
  ...
  void Window::setFrameRate(int limit)
  {
    m_windowContext.window.setFramerateLimit(limit);
  }

  void Window::subscribeEvent(std::function<void(WindowContext&)> closure)
  {
    m_closureEvent = std::move(closure);
  }
  void Window::unsubscribeEvent()
  {
    m_closureEvent = {};
  }

  void Window::subscribeUpdate(std::function<void(WindowContext&)> closure)
  {
    m_closureUpdate = std::move(closure);
  };
  void Window::unsubscribeUpdate()
  {
    m_closureUpdate = {};
  };

  void Window::subscribeDisplay(std::function<void(WindowContext&)> closure)
  {
    m_closureDisplay = std::move(closure);
  };
  void Window::unsubscribeDisplay()
  {
    m_closureDisplay = {};
  };
