 ...
 *
 * This structure contains the client identifier, command, and length of the
 * data that follows in the message.
 */
struct MessageHeader {
 ...
 *
 * This structure combines the MessageHeader with the actual data payload,
 * allowing for easy manipulation and transmission of network messages.
 */
struct Message {
  MessageType const type = MessageType::Message; ///< Type of the message.
 ...
 *
 * This structure is used to transmit client's input, typically a single
 * character.
 */
struct InputData {
 ...
 *
 * This structure is used when the server sends a notification that a client has
 * disconnected.
 */
struct ServerClientDisconnectedData {
