...
bool Client::WelcomeScene::isInShape(sf::Event::MouseButtonEvent& mouse, sf::RectangleShape& myShape) {
...
int Client::WelcomeScene::pollEvent(sf::RenderWindow& window, Client::Network& network) {
  (void)network;
  while (window.pollEvent(this->event_)) {
    ...
    if (this->event_.type == sf::Event::MouseButtonPressed || this->event_.type == sf::Event::MouseButtonReleased) {
