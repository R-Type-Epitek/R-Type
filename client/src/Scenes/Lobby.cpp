...
bool Client::LobbyScene::isInShape(
...
int Client::LobbyScene::pollEvent(
  sf::RenderWindow &window,
  Client::Network &network)
{
  while (window.pollEvent(this->event_)) {
    if (
      this->event_.type == sf::Event::Closed ||
      (this->event_.type == sf::Event::KeyPressed &&
       this->event_.key.code == sf::Keyboard::Escape)) {
      // TODO: open pop up to ask if user wants to quit if he is in a
      // lobby
      window.close();
    }
    ...
    if (
      this->event_.type == sf::Event::TextEntered &&
      this->event_.text.unicode < 128) {
      if (this->ipInput_->isClicked() && !this->portInput_->isClicked()) {
        ...
        if (this->event_.text.unicode != 13) { // 13 = Enter
          ...
          this->ipInput_->setInput(
            static_cast<char>(this->event_.text.unicode));
        } else {
    ...
    // get mouse event
    if (
      (this->event_.type == sf::Event::MouseButtonPressed) ||
      (this->event_.type == sf::Event::MouseButtonReleased)) {
      // Button click
      if (this->isInShape(
            this->event_.mouseButton,
            *this->escapeButton_->getShape())) {
        if (this->event_.type == sf::Event::MouseButtonReleased) {
          this->escapeButton_->isReleased();
          // TODO: open pop up to ask if user wants to quit if he is
          // in a lobby
          window.close();
        }
