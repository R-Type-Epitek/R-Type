...
Client::Network::Network(std::string ip, std::string port) : socket(io) {
  boost::asio::ip::udp::resolver resolver(this->io);
  this->receiverEndpoint = *resolver.resolve(boost::asio::ip::udp::v4(), ip, port).begin();
...
void Client::Network::send(boost::asio::const_buffer const& buffer) {
  this->socket.send_to(buffer, this->receiverEndpoint);
}

void Client::Network::sendMessage(std::string const& command, char const* data, size_t dataSize) {
  ...
  if (dataSize > 0) memcpy(messageBuffer.data() + sizeof(header), data, dataSize);

  this->send(boost::asio::buffer(messageBuffer.data(), messageBuffer.size()));
}

Response* Client::Network::receiveAndValidateResponse(std::string const& expectedCommand) {
  ...
  if (response->header.status != 200) throw std::runtime_error(response->header.statusMessage);
...
void Client::Network::updateName(std::string name) {
  if (name.length() > MAX_NAME_LENGTH) throw std::runtime_error("The name is too long");
...
void Client::Network::setClientId(int id) { this->clientId = id; }

int Client::Network::getClientId() const { return this->clientId; }

void Client::Network::setRoomId(int id) { this->roomId = id; }

int Client::Network::getRoomId() const { return this->roomId; }

void Client::Network::setName(std::string name) { this->name = name; }

std::string Client::Network::getName() const { return this->name; }
