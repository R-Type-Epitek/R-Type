 ...
 * It uses a mutex and a condition variable to ensure thread safety during push and pop operations.
 *
...
class ThreadSafeQueue {
 public:
   ...
   *
   * This method waits for an element to be available if the queue is empty.
   * It locks the queue, pops the element, and then unlocks it.
   *
 ...
 private:
  ...
  std::mutex mutex;              ///< Mutex for ensuring thread safety during push and pop operations.
  std::condition_variable cond;  ///< Condition variable used to wait for and notify about queue operations.
};
}  // namespace Network
