...
boost::asio::ip::udp::socket& Network::UDPServer::getSocket() { return this->socket; }

std::vector<Network::Client> Network::UDPServer::getClients() const { return this->clients; }

boost::array<char, 1024> Network::UDPServer::getRecvBuffer() const { return this->recvBuffer; }

boost::asio::ip::udp::endpoint Network::UDPServer::getRemoteEndpoint() const { return this->remoteEndpoint; }

std::vector<Network::Room> Network::UDPServer::getRooms() const { return this->rooms; }

void Network::UDPServer::addClient(Network::Client client) { this->clients.push_back(client); }
...
void Network::UDPServer::startReceive() {
  this->socket.async_receive_from(
      boost::asio::buffer(this->recvBuffer), this->remoteEndpoint,
      boost::bind(&Network::UDPServer::handleReceive, this, boost::asio::placeholders::error,
...
void Network::UDPServer::sendToAllClientsInRoom(boost::asio::const_buffer const& buffer, int roomId) {
  if (roomId < 0 || roomId >= (int)this->rooms.size()) throw std::runtime_error("Invalid room id");
...
void Network::UDPServer::sendToClient(boost::asio::const_buffer const& buffer, int id) {
  if (id < 0 || id >= (int)this->clients.size()) throw std::runtime_error("Invalid client id");
  if (!this->socket.is_open()) throw std::runtime_error("Socket is not open");
...
void Network::UDPServer::processMessage(TimedMessage timedMessage) {
  ...
  if (it != this->commandHandlers.end()) {
    if (!it->second->isAuthorized(message->header.clientId)) return this->handleInvalidClient(timedMessage);
...
Response Network::UDPServer::createResponse(int clientId, std::string const& command, std::string const& statusMessage,
                                            int status) {
  Response response;
...
void Network::UDPServer::sendResponseAndLog(TimedMessage timedMessage, Response const& response) {
  int const width = 20;
  auto end = std::chrono::high_resolution_clock::now();
  auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - timedMessage.timestamp).count();

  this->sendToClient(boost::asio::buffer(&response, sizeof(response)), response.header.clientId);
...
void Network::UDPServer::handleInvalidClient(TimedMessage timedMessage) {
  ...
  std::string statusMessage =
      clientId < 0 ? "Invalid client ID" : "Client not connected. Use " HELLO_COMMAND " command to connect";

  Response response =
      this->createResponse(clientId, timedMessage.message->header.command, statusMessage, RES_UNAUTHORIZED);
...
void Network::UDPServer::handleInvalidCommand(TimedMessage timedMessage) {
  Response response = this->createResponse(timedMessage.message->header.clientId, timedMessage.message->header.command,
                                           "Invalid command", RES_BAD_REQUEST);
  return this->sendResponseAndLog(timedMessage, response);
}
